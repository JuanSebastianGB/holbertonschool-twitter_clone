...
class Post {
  ...
  Future<List<Post>> getAllPostList() async {
    ...
    final allData =
        querySnapshot.docs.map((doc) => doc.data()).cast<Post>().toList();
  ...
  Future setPostLike(Post post, String userID) async {
    ...
    await postRef
        .doc(postID)
        .update({'likeList': post.likeList, 'likeCount': post.likeList.length});
    debugPrint('executed like');
  }
